...
def make_scalar_plot_old(period_list,series,fieldDesc,yLab,axis,colorStyle):
    ...
    plt.gca().xaxis.grid(b=True, which=u'both')
    plt.gca().yaxis.grid(b=True, which=u'both',color=ylabelColor[0],linewidth=1)
    ...
    ax1.legend(loc=2, prop={'size':14},handlelength=3,numpoints=4)
    if (isax2 == True):
        box2 = ax2.get_position()
        ax2.set_position([box2.x0, box2.y0 + box2.height * 0.15,
                 box2.width, box2.height * 0.85])
        ax2.legend(loc=1, prop={'size':14}, handlelength=3,numpoints=4)

    plt.show()

    return

def make_scalar_plot(period_list,series,fieldDesc,yLab,axis,colorStyle):
